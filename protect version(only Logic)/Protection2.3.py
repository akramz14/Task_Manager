...
class Logic:
	def __init__(self):
		...
		self.id = 0
	#date Error
	def dateError(self,value,fmt="%Y-%m-%d %H:%M:%S") :
		try:
	...
	def EditStatus(self,id,status=None):
		...
		if status is not Status.UNFINISHED or is not Status.WORKING:
			raise TypeError("خطأ يجب ان تختار حالة صحيحه (خطأ مستحيل يظهر لمستخدم اصلا🤓 )")
		if status == Status.UNFINISHED:
			if self.tasks[id]["status"] == Status.FINISHED:
				raise ValueError("لا يمكنك تحويل مهمة مكتلمة الى غير مكتلمة")
			self.tasks[id]["status"] = Status.WORKING
		
		if status == Status.WORKING:
			if self.tasks[id]["status"] == Status.UNFINISHED:
				raise ValueError("لا يمكنك تحويل مهمة قيد انجاز الى غير مكتلمة")
			self.tasks[id]["status"]=Status.FINISHED
	#edit endtask
	def EditETask(self,id,end_task):
		self.dateError(end_task)
		self.idError(id)
		if self.tasks[id]["status"] == Status.UNFINISHED:
