...
class Logic:
	...
	def idError(self,id):
		...
		if id not in self.tasks:
			raise ValueError("خطأ مهمة غير موجودة")
	#Date error
	def now(self):
	...
	def EditStatus(self,id,status=None):
		...
		if status == Status.WORKING:
			if self.tasks[id]["status"] == Status.FINISHED:
				raise ValueError("لا يمكنك تحويل مهمة مكتلمة الى قيد انجاز")
			self.tasks[id]["status"]=Status.WORKING
		
		if status == Status.FINISHED:
			if self.tasks[id]["status"] == Status.UNFINISHED:
				raise ValueError("لايمكنك تحويل مهمة غير مكتلمة مباشرة الى مكتملة")
